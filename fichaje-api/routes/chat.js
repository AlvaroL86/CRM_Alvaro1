...
async function isAdmin(roomId, userId) {
 ...
 *                     UPLOAD  (MULTIPART)
 * =======================================================*/
router.post("/upload", authMW, upload.array("files", 10), async (req, res) => {
  try {
    const files = (req.files || []).map((f) => {
      const publicUrl = `/uploads/chat/${f.filename}`;
      const mime = f.mimetype || mimeLookup(f.filename) || "application/octet-stream";
      let tipo = "archivo";
...
router.get("/room/:id/members", authMW, getMembersHandler);
router.get("/rooms/:id/members", authMW, getMembersHandler);

async function patchRoleHandler(req, res) {
  try {
    const roomId = String(req.params.id);
    const target = String(req.params.userId);
    const rol = req.body?.rol === "admin" ? "admin" : "miembro";

    if (!(await isAdmin(roomId, req.user.id))) {
      return res.status(403).json({ error: "no_admin" });
    }
...
router.patch("/room/:id/members/:userId", authMW, patchRoleHandler);
router.patch("/rooms/:id/members/:userId", authMW, patchRoleHandler);
...
router.delete("/room/:id/members/:userId", authMW, deleteMemberHandler);
router.delete("/rooms/:id/members/:userId", authMW, deleteMemberHandler);
...
async function addMembersHandler(req, res) {
  try {
    ...
    console.error("POST add members", e);
    res.status(500).json({ error: "No se pudo invitar" });
  }
}
router.post("/room/:id/members", authMW, addMembersHandler);
router.post("/rooms/:id/members", authMW, addMembersHandler);
...
router.post("/rooms/private", authMW, async (req, res) => {
  try {
    ...
    const [[yo]] = await db.query(
      `SELECT id,nif,nombre FROM usuarios WHERE id=? LIMIT 1`,
      [me]
    );
   ...
   LEFT JOIN usuarios u ON u.id = m.user_id
    ...
    res.json(
      rows.map((r) => ({
        ...
        from: { id: r.user_id, nombre: r.from_nombre },
      }))
    );
  } catch (e) {
...
router.get("/users", authMW, async (req, res) => {
  try {
    ...
    if (!rows.length) {
      ...
      if (q) {
        sql2 += ` AND (nombre LIKE ? OR email LIKE ?)`;
        params2.push(`%${q}%`, `%${q}%`);
      }
